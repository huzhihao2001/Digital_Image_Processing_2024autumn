...
def create_mask_from_points(points, img_h, img_w):
    ...
    mask1=torch.from_numpy(mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255
    boundary = (F.conv2d(mask1, kernel, padding=1).squeeze() > 0).float().nonzero()
    return mask,boundary

# Calculate the Laplacian loss between the foreground and blended image
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask,foreground_boundary,background_boundary):
    """
    ...
    filters = torch.tensor([[0, 1, 0],
                       [1, -4, 1],
                       [0, 1, 0]], dtype=torch.float32).to(device).unsqueeze(0).unsqueeze(0).repeat(3,1,1,1)
    ...
    loss=F.mse_loss(foreground_test[0,:,mask1[:,0],mask1[:,1]],blended_test[0,:,mask2[:,0],mask2[:,1]])+\
        100*F.mse_loss(foreground_img[0,:,foreground_boundary[:,0],foreground_boundary[:,1]],blended_img[0,:,background_boundary[:,0],background_boundary[:,1]])
    return loss
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    foreground_mask,foreground_boundary = create_mask_from_points(foreground_polygon_points, foreground_np.shape[0], foreground_np.shape[1])
    background_mask,background_boundary = create_mask_from_points(background_polygon_points, background_np.shape[0], background_np.shape[1])
    ...
    for step in range(iter_num):
        ...
        loss = cal_laplacian_loss(fg_img_tensor, fg_mask_tensor, blended_img_for_loss, bg_mask_tensor,foreground_boundary,background_boundary)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
