    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('turn left', width=viz.button_w, enabled='image' in viz.result):
                    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
                    preds=fa.get_landmarks(self.viz.result.image[:,:,:3])
                    preds=preds[0]
                    self.feature_points=[pred[:2] for pred in preds]
                ...
                imgui.same_line()
                if imgui_utils.button('turn right', width=viz.button_w, enabled='image' in viz.result):
                    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
                    preds=fa.get_landmarks(self.viz.result.image[:,:,:3])
                    preds=preds[0]
                    self.feature_points=[pred[:2] for pred in preds]
                ...
                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('turn up', width=viz.button_w, enabled='image' in viz.result):
                    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
                    preds=fa.get_landmarks(self.viz.result.image[:,:,:3])
                    preds=preds[0]
                    self.feature_points=[pred[:2] for pred in preds]
                ...
                imgui.same_line()
                if imgui_utils.button('turn down', width=viz.button_w, enabled='image' in viz.result):
                    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
                    preds=fa.get_landmarks(self.viz.result.image[:,:,:3])
                    preds=preds[0]
                    self.feature_points=[pred[:2] for pred in preds]
